    ...
    try {
        for (const CommandDescription& command : commands_) {
            if (command.command == "Stop") {
                Coordinates coor = ParseCoordinates(command.description);
                catalogue.AddStop(command.id,coor.lat, coor.lng);
            }
        }
        for (const CommandDescription& command : commands_) {
            if (command.command == "Bus") {
                auto parsed_route = ParseRoute(command.description);
                catalogue.AddRoute(command.id, parsed_route, IsItCircleRoute(command.description));
            }
        }
